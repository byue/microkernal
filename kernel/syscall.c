...
int argstr(int n, char **pp) {
  ...
  *pp = fetcharg(n);
  if (*pp != NULL) {
    // max string length is 510 by C89 standards
    for (int i = 0; i < 510; i++) {
      if ((*pp)[i] == '\0') {
        return i;
      }
  ...
  return -1;
  // ls doesn't work with fetchstr because of inconsistent use of vspace
...
extern int sys_app_syscall(void);
extern int sys_fork_guest(void);
extern int sys_gnum_children(void);
extern int sys_gnext_syscall(void);
extern int sys_gresume(void);
extern int sys_gquery_user_pages(void);
extern int sys_grequest_proc(void);
extern int sys_gload_program(void);
extern int sys_gaddmap(void);
extern int sys_gremovemap(void);
extern int sys_gupdate_flags(void);
extern int sys_gdeploy_program(void);
...
static int (*syscalls[])(void) = {
    ...
    [SYS_app_syscall] = sys_app_syscall, [SYS_gnum_children] = sys_gnum_children,
    [SYS_gnext_syscall] = sys_gnext_syscall, [SYS_gresume] = sys_gresume,
    [SYS_gquery_user_pages] = sys_gquery_user_pages, [SYS_fork_guest] = sys_fork_guest,
    [SYS_grequest_proc] = sys_grequest_proc, [SYS_gload_program] = sys_gload_program,
    [SYS_gaddmap] = sys_gaddmap, [SYS_gremovemap] = sys_gremovemap,
    [SYS_gupdate_flags] = sys_gupdate_flags, [SYS_gdeploy_program] = sys_gdeploy_program,
};
