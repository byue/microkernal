...
#define NDISK 9        // number of user disks
...
struct superblock {
  ...
  int cids[NDISK]; // Keeps track of owner of each user disk,
                   // or -1 if not used
  uint udiskstart; // Block number of the start of user disk
  // TODO: we don't the following two params!
  uint bmapstart;  // Block number of first free map block
  uint inodestart; // Block number of the start of inode file
  uint freeblock;  // Block number of the first free block
};
